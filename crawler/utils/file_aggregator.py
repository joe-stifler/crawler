...
ignored_folders = ['third_party', '.git', 'scans', 'build', '.vscode', '.github', 'transformed_images']
...
def get_directory_structure(startpath):
    ...
    for root, dirs, files in os.walk(startpath, topdown=True):
        dirs[:] = [d for d in dirs if os.path.join(root, d) not in [os.path.join(startpath, f) for f in ignored_folders]]  # Modify dirs in-place to ignore certain directories
...
print(f"All .h and .cpp files have been aggregated into {output_file}, ignoring specified folders, with directory structure prepended.")
