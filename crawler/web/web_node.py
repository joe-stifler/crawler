...
class WebNode(BaseNode):
    """
    Represents a node in the web graph, corresponding to a web page. It supports lazily fetching HTML content, parsing it,
    extracting hyperlinks, and converting the content to Markdown.
    ...
    **attributes : dict, optional
    ...
    def soup(self):
        """
        A property that ensures the HTML content is fetched and parsed upon first access. It returns a BeautifulSoup
        object containing the parsed HTML of the web page. This allows for lazy loading of web page content, minimizing
    ...
    def fetch_connected_hyperlinks(self):
        """
        Extracts and returns all hyperlinks found within the web page's HTML content. It parses the `a` tags to extract
        the `href` attribute values, resolving them to absolute URLs.
    ...
    def convert_to_markdown(self):
        """
        Converts the web page's HTML content to Markdown format using the html2text library. This method allows for
        a text representation of the web page's content, which can be particularly useful for documentation or
    ...
    def url(self):
        """
        A property returning the URL of the web page this node represents. It provides direct access to the node's
        identifier, which in the context of a `WebNode`, is the URL.
    ...
    def domain(self):
        """
        Extracts and returns the domain part of the web page's URL, facilitating operations that require domain-level
        granularity, such as restricting crawling activities to specific domains.
    ...
    def __str__(self):
        """
        Provides a human-readable string representation of the WebNode, primarily for debugging and logging purposes.
    ...
    def __repr__(self):
        """
        Provides a detailed string representation of the WebNode, suitable for interactive use in the interpreter.
    ...
    def _repr_html_(self, index=None):
        """
        Generates an HTML representation of the WebNode for display in Jupyter notebooks or web interfaces
        , incorporating additional metadata such as the node's index, depth, parent, and domain for enhanced visualization.
    ...
    def to_markdown(self):
        """
        Converts the node's content, specifically the web page's HTML content, to Markdown format. This method leverages
        the `convert_to_markdown` method to produce a Markdown representation of the HTML content, facilitating its use in
        documentation, notes, or any other context where Markdown is preferred.
